...
it('reads options from React children of <RadioSelectOption>s', () => {
  const singleOption = <RadioSelectOption label="Foo" value="foo" />;
  ...
  const multipleOptions = [
    <RadioSelectOption label="Foo" value="foo" checked />,
    <RadioSelectOption label="Bar" value="bar" readOnly />,
    <RadioSelectOption label="Meh" value="meh" />,
  ];
  expect(parseSelectOptions(multipleOptions)).toMatchObject([
    { label: 'Foo', value: 'foo', checked: true },
    { label: 'Bar', value: 'bar', readOnly: true },
